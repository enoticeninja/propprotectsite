...
include_once DIR_CORE_CLASSES.'Class.ImageManipulator.php';
?>
<?php
...
if(!is_dir(DIR_IMAGE_UPLOADS.'50X50')){
  mkdir(DIR_IMAGE_UPLOADS.'50X50');
}
if(!is_dir(DIR_IMAGE_UPLOADS.'100X100')){
  mkdir(DIR_IMAGE_UPLOADS.'100X100');
}
...
function commonResizeProduct($data){
    global $db_conx,$uploads_path,$frontend_site_path,$date_created,$undo_path;
    $thumbnail = $image = $data['image'];
    $sql = "INSERT INTO images (image,thumbnail,date_created)
    ...
    $quotient = (int)($imageid/1000);
    $sql_update = "UPDATE images SET path='$quotient/$imageid' WHERE id='$imageid'";
    $query_update = mysqli_query($db_conx, $sql_update);  

	$sql2 = "INSERT INTO product_image (image_id,product_id,sort_order)
    VALUES('$imageid','$data[product_id]',999)";
	$query2 = mysqli_query($db_conx,$sql2);
	$return['product_image_id'] = mysqli_insert_id($db_conx);	
    if(!is_dir(DIR_IMAGE_UPLOADS.$quotient.'/'.$imageid)){
      mkdir(DIR_IMAGE_UPLOADS.$quotient.'/'.$imageid, 0777, true);
    }       

    rename($uploads_path.$image, $uploads_path.$quotient.'/'.$imageid.'/'.$image);
        ...
        ImageManagerCore::$dimensions['300X300-'] = array('300','300');
        ImageManagerCore::$dimensions['400X400-'] = array('400','400');
        ImageManagerCore::$dimensions['500X500-'] = array('500','500');
        ImageManagerCore::$dimensions['600X600-'] = array('600','600');	
		//print_r(ImageManagerCore::$dimensions);
    ImageManagerCore::resize($uploads_path.$quotient.'/'.$imageid.'/'.$image, $uploads_path.$quotient.'/'.$imageid.'/', 50, 50, 'jpg',false,$image); 
    $return['imageid'] = $imageid;
    $return['image'] = $image;
    $return['path'] = $quotient.'/'.$imageid;
    return $return;    
}

function resizeImageForProduct($data=array()){
    global $db_conx,$uploads_path,$frontend_site_path,$date_created,$undo_path;
    $image = $_POST['imagename'];
    $guid = $_POST['guid'];
    $tempReturn = commonResizeProduct($image);
    $imageid = $tempReturn['imageid'];
    $return['html'] = htmlForProductImageTable($tempReturn,$_POST['action']);
...
/* 		$iman = new ImageManagerCore();
    ...
    if ($_POST['action'] == 'crop'){
     ...
     if ($_POST['action'] == 'undo'){
            ...
            rename($undo_path."$guid/$previous_image", $uploads_path.$path.$image);
            list($widthNew, $heightNew) = getimagesize($uploads_path.$path.$image);
            $return['width'] = $widthNew;
            $return['height'] = $heightNew;
            $return['height'] = $heightNew;
