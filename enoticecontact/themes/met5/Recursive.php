...
if(isset($_GET['currentParent'])){
	...
	if($num_rows > 0){
		...
		$tableOptions = $class->getTableFieldsCustom($currentParent,$currentLevelDepth,$nLeft,$nRight);
	} 
	else{
		$tableOptions = $class->getTableFieldsCustom(null,0,null,null);                
	}            
}
else{
	$tableOptions = $class->getTableFieldsCustom(null,0,null,null);
					...
					<div class="d-flex align-items-center">
						...
						if(isset($currentParent)){
							...
							';
								foreach($parents as $parent){
									if($parent['id'] != $currentParent){
										...
										<li class="m-nav__item">
											<a href="'.$currentUrl.'?currentParent='.$parent['id'].'" class="m-nav__link">
								...
								$breadcrumbs .= '<li class="m-nav__item">'.$parents[$currentParent]['name'].'</li>';
								$breadcrumbs .= '</ul>';
								echo $breadcrumbs;                                    
						}
						?>
...
jQuery(document).ready(function() {
	...
	table1.createTableRow = function($data, $userData, $trclass=''){
		$trid = $userData[table1.config['id']];
		var $table = $.extend(true,{},table1.tableConfig);		
        var $row_data = ''; 
        var $icon = ''; 
        if(+$data['nright'] == +$data['nleft'] + 1){
            $icon = '<i class="fa fa-file m--font-warning"></i>'; 
            $icon2 = ''; 
            $onClickFunc = '';
            $row_data += 
            '\
            <tr class=" mt-element-ribbon '+$trclass+' '+table1.module+'-tr" id="tr-'+$trid+'">\
            ';	            
            
        }  
        else{
            $icon = '<i class="fa fa-folder m--font-success"></i>'; 
            $icon2 = '<i class="fa fa-ellipsis-h m--font-info"></i>';
            $onClickFunc = 'onClick="RouteToAnotherLevel('+$data['id']+')" style="cursor:pointer"';            
            $row_data += 
            '\
            <tr class=" mt-element-ribbon '+$trclass+' '+table1.module+'-tr" id="tr-'+$trid+'">\
            ';	            
		...
		$.each($data,function($key,$col){
			var $tdval = $data[$key];
			if($key == 'name') $tdval = $icon+'  '+$col+'  '+$icon2;
			...
			$row_data += 
			'\
			<td class="td-'+$key+'" id="td-'+$trid+'-'+$key+'" '+$onClick+'>'+$tdval+'</td>\
			';
		});
        $row_data += 
        '\
