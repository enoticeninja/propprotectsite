...
class ImageManagerCore
{
	...
	public function __construct(){
        self::$dimensions['100X100-'] = array('100','100');
        self::$dimensions['200X200-'] = array('200','200');
        self::$dimensions['300X300-'] = array('300','300');
        self::$dimensions['400X400-'] = array('400','400');
        self::$dimensions['500X500-'] = array('500','500');
        self::$dimensions['600X600-'] = array('600','600');
	}
    /**
    ...
    public static function thumbnail($image, $cache_image, $size, $image_type = 'jpg', $disable_cache = true, $regenerate = false)
    {
        ...
        if ($regenerate || !file_exists(_PS_TMP_IMG_DIR_.$cache_image)) {
            ...
            if (!ImageManagerCore::checkImageMemoryLimit($image)) {
                return false;
            }
            ...
            // We need to resize */
                ...
                ImageManagerCore::resize($image, _PS_TMP_IMG_DIR_.$cache_image, $ratio_x, $size, $image_type);
            }
        }
...
/*     public static function resize($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg',
                                ...
                                &$src_width = null, &$src_height = null)
    {
        ...
        if (!ImageManagerCore::checkImageMemoryLimit($src_file)) {
        ...
        if ($file_type == 'png' && $type == IMAGETYPE_PNG) {
            ...
            $return['imagemanager'][$src_file]['file_type'] = $file_type;
            $return['imagemanager'][$src_file]['type'] = $type;
            $return['imagemanager'][$src_file]['dst_width'] = $dst_width;
            $return['imagemanager'][$src_file]['dst_height'] = $dst_height;

        $src_image = ImageManagerCore::create($type, $src_file);
        if ($rotate) {
        ...
        if ($dst_width >= $src_width && $dst_height >= $src_height) {
            ...
            ImageManagerCore::imagecopyresampled($dest_image, $src_image, (int)(($dst_width - $next_width) / 2), (int)(($dst_height - $next_height) / 2), 0, 0, $next_width, $next_height, $src_width, $src_height, $quality);
        }

        $write_file = ImageManagerCore::write($file_type, $dest_image, $dst_file);
  ...
  */
    ...
    public static function resize($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg',
                                $force_type = false,$original_name, &$error = 0, &$tgt_width = null, &$tgt_height = null, $quality = 5,
                                &$src_width = null, &$src_height = null)
    {
        global $return,$uploads_path;
		$return['manipulator_log'][] = 'Source File: '.$src_file;
		$return['manipulator_log'][] = 'Dest File: '.$dst_file;
		//print_r(self::$dimensions);
        if (PHP_VERSION_ID < 50300) {
			$return['manipulator_error'][] = 'PHP LESS THAN 50300';
            clearstatcache();
        } else {
			$return['manipulator_error'][] = 'PHP GREATER THAN 50300';
            clearstatcache(true, $src_file);
        }

        if (!file_exists($src_file) || !filesize($src_file)) {
			$return['manipulator_error'][] = self::ERROR_FILE_NOT_EXIST;
            return !($error = self::ERROR_FILE_NOT_EXIST);
        }
        ...
        if (!$src_width) {
			$return['manipulator_error'][] = self::ERROR_FILE_WIDTH;
            return !($error = self::ERROR_FILE_WIDTH);
        }
        if (!$dst_width) {
            $dst_width = $src_width;
...
/*         if (!ImageManagerCore::checkImageMemoryLimit($src_file)) {
        ...
        foreach($dimensions as $key => $dimension){
            //$src_image = $src_image_main;
            $dstination_file = $dst_file.'/'.$key.$original_name;
            $dst_width = $dimension[0];
            $dst_height = $dimension[1];
            /* $return['imagemanager'][$src_file]['dimensions'] = $dimension; */
            $width_diff = $dst_width / $src_width;
            $height_diff = $dst_height / $src_height;
...
/*                 $return['imagemanager'][$src_file]['file_type'] = $file_type;
            ...
            $write_file = ImageManagerCore::write($file_type, $dest_image, $dstination_file);
...
/*     public static function validateIconUpload($file, $max_file_size = 0)
    ...
    public static function cut($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $dst_x = 0, $dst_y = 0)
    {
        ...
        $src = array(
            'width' => $src_info[0],
            'height' => $src_info[1],
            'ressource' => ImageManagerCore::create($src_info[2], $src_file),
        );
        ...
        $dest['ressource'] = ImageManagerCore::createWhiteImage($dest['width'], $dest['height']);

        $white = imagecolorallocate($dest['ressource'], 255, 255, 255);
        imagecopyresampled($dest['ressource'], $src['ressource'], 0, 0, $dest['x'], $dest['y'], $dest['width'], $dest['height'], $dest['width'], $dest['height']);
        imagecolortransparent($dest['ressource'], $white);
        $return = ImageManagerCore::write($file_type, $dest['ressource'], $dst_file);
    ...
    public static function write($type, $resource, $filename)
    {
        ...
        $return['imagemanager'][$filename]['timeelapsed'][$type] = $time_elapsed_secs; 
