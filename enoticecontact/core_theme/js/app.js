...
var App = function() {
    var t, e = !1,
        ...
        f = function() {
            ...
            }),$("body").on("click", ".modal .modal-button-group > .fullscreen", function(t) {
                t.preventDefault();
                var e = $(this).closest(".modal");
                if (e.hasClass("portlet-fullscreen")){ $(this).removeClass("on"), e.removeClass("portlet-fullscreen"), $("body").removeClass("page-portlet-fullscreen"), e.children(".modal-body").css("height", "auto"),e.children(".modal-dialog").removeClass('modal-full');
				e.find(".fullscreen>i").addClass('fa-expand');
				e.find(".fullscreen>i").removeClass('fa-compress');					
				e.addClass('top-10per');				
				}
                else {
					e.children(".modal-dialog").addClass('modal-full');
					e.removeClass('top-10per');	
					e.find(".fullscreen>i").removeClass('fa-expand');
					e.find(".fullscreen>i").addClass('fa-compress');
                    var o = App.getViewPort().height - e.children(".modal-header").outerHeight() - parseInt(e.children(".modal-body").css("padding-top")) - parseInt(e.children(".modal-body").css("padding-bottom"));
                    $(this).addClass("on"), e.addClass("portlet-fullscreen"), $("body").addClass("page-portlet-fullscreen"), e.children(".modal-body").css("height", o)
                }
            }), $("body").on("click", ".portlet > .portlet-title > .tools,.actions > a.reload", function(t) {
                t.preventDefault();
        ...
        b = function() {
            if ($().uniform) {
                var t = $("input[type=checkbox]:not(.toggle, .md-check, .md-radiobtn, .make-switch, .icheck, .togglebutton-paper), input[type=radio]:not(.toggle, .md-check, .md-radiobtn, .star, .make-switch, .icheck)");
                t.length > 0 && t.each(function() {
                    0 === $(this).parents(".checker").length && ($(this).show(), $(this).uniform())
                })
            }
        },
        g = function() {
    ...
    return {
        ...
        scrollTo: function(t, e) {
            ...
            o -= $(".common-header-calculation").height();
            var elemTop = t[0].getBoundingClientRect().top;
            var elemBottom = t[0].getBoundingClientRect().bottom;
            var isVisible = (elemTop >= $(".common-header-calculation").height()) && (elemBottom <= window.innerHeight);            
            if(!isVisible){
                $("html,body").animate({
                    scrollTop: o-20
                }, "slow")                
