var TableExcel = function(options){
	...
	$this.select = function($dataArray){
		...
		$input = 
		'\
		<select class=" td-'+$dataArray['col']+' data-field="'+$dataArray['col']+'" data-id="'+$dataArray['id']+'">\
		'+$options1+'\
		</select>\
		';
		return $input;
        ...
        $.each($table['cols'],function($kk,$col){
			if(!$this.isCreatingNewRow){
				if($fKeys[$col]){ //// Replace foreign keys with _name prefix
					//$col_data_temp[$col] = '<input class="excel-cell-input" type="text" value="'+$data[''+$col+'_name']+'" disabled>';
					//$col_data_temp[$col] = $data[''+$col+'_name'];
					if(focusChildNode && $table['editable'][$col]){
						if($table['editable'][$col]['type'] == 'input'){
							elemArray = {value:$data[''+$col+'_name'],col:$col,id:$data['id']};
							$col_data_temp[$col] = $this.input(elemArray);
						}
						if($table['editable'][$col]['type']  == 'select'){
							elemArray = {value:$data[''+$col+''],col:$col,id:$data['id'],options:$table['editable'][$col]['options']};
							$col_data_temp[$col] = $this.select(elemArray);
						}
						//$col_data_temp[$col] = '<input class="excel-cell-input on-change-update-cell td-'+$col+' autoselect-on-focus" type="text" value="'+$data[''+$col+'_name']+'" data-field="'+$col+'" data-id="'+$data['id']+'" >';
					}
					else{
						if(!isCreatingEmptyRow) $col_data_temp[$col] = $data[''+$col+'_name'];
						else $col_data_temp[$col] = '';
					}
				}
				else{
					if(focusChildNode  && $table['editable'][$col]){
						//console.log($table['editable'][$col]['type']);
						//console.log($col);
						/* $col_data_temp[$col] = '<input class="excel-cell-input on-change-update-cell td-'+$col+' autoselect-on-focus" type="text" value="'+$data[$col]+'" data-field="'+$col+'" data-id="'+$data['id']+'">'; */
						if($table['editable'][$col]['type'] == 'input'){
							elemArray = {value:$data[$col],col:$col,id:$data['id']};
							$col_data_temp[$col] = $this.input(elemArray);
						}
						if($table['editable'][$col]['type']  == 'select'){
							elemArray = {value:$data[$col],col:$col,id:$data['id'],options:$table['editable'][$col]['options']};
						...
						if(!isCreatingEmptyRow) $col_data_temp[$col] = $data[$col];
						else $col_data_temp[$col] = '';
					}
				...
				/* if(i >= totalCols)$col_data_temp[$col] += '<span class="table-task-buttons"><a class="btn btn-circle btn-icon-only bg-less-glossy-error" href="javascript:;""><i class="flaticon-cancel" style="font-size: 30px;"></i></a></span>'; */
			}
			i++;
        });
	...
	function validateData($userData){
		...
		check['status'] = true;
		check['fields'] = [];
		if(($userData['desired_spot'] >= $userData['spots'])){
			check['status'] = false;
			check['fields'].push('desired_spot');
			check['fields'].push('spots');
		}
		if(($userData['desired_spot'] == 0)|| ($userData['spots'] == 0)){
			check['status'] = false;
			check['fields'].push('desired_spot');
			check['fields'].push('spots');
		}
		...
		if(($userData['desired_spot'] == 0)){
			check['status'] = false;
			check['fields'].push('desired_spot');
		}
		if($userData['start_time'] >= $userData['end_time']){
			check['status'] = false;
			check['fields'].push('start_time');
			check['fields'].push('end_time');
		}
		if($userData['start_time'] == '00:00:00'){
			check['status'] = false;
			check['fields'].push('start_time');
		}
		if($userData['same_break'] > 0){
			check['status'] = false;
			check['fields'].push('same_break');
		}
		return check;
	}
    $this.createTableRow = function($data, $userData, $trclass){
		...
		var check = validateData($userData);
